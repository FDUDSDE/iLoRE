...
parser.add_argument('-d', '--data', type=str, help='Dataset name (eg. wikipedia or reddit)',default='reddit')
...
parser.add_argument('--head_number', type=int, default=64, help='Nunber of Multi-head Attention')
...
MODEL_SAVE_PATH = f'./saved_models/{args.data}/{args.prefix}-{args.data}8' + '\
  node-classification.pth'
get_checkpoint_path = lambda \
    epoch: f'./saved_checkpoints/{args.data}/{args.prefix}-{args.data}8-{epoch}' + '\
  node-classification.pth'
...
for i in range(args.n_runs):
  ...
  model_path = f'./saved_models/{args.data}/{args.prefix}-{DATA}6.pth'
  tgn.load_state_dict(torch.load(model_path))
  tgn.eval()
  logger.info('TGN models loaded')
  logger.info('Start training node classification task')

  decoder = MLP(MEMORY_DIM, drop=DROP_OUT)
  ...
  for epoch in range(args.n_epoch):
    ...
    val_auc = eval_node_classification(tgn, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
                                       n_neighbors=NUM_NEIGHBORS, mini_batch_size = MINI_BATCH_SIZE)
